// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplateworkshops = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api/v1/event/workshops": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Get workshops",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Workshops"
                ],
                "summary": "Get workshops",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Page",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Size page",
                        "name": "size_page",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by event id",
                        "name": "event_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filter by workshop type id",
                        "name": "type_id",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/rest.workshopsResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Create workshop",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Workshops"
                ],
                "summary": "Create workshop",
                "parameters": [
                    {
                        "description": "Create workshop body",
                        "name": "createWorkshopBody",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/rest.createWorkshopValidated"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/httpResponse.IdResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            }
        },
        "/api/v1/event/workshops/summary": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Get workshop sums",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "WorkshopSums"
                ],
                "summary": "Get workshop sums",
                "parameters": [
                    {
                        "type": "string",
                        "description": "the id of workshop",
                        "name": "workshop_id",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/rest.workshopsSummaryResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            }
        },
        "/api/v1/event/workshops/{workshopId}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Get workshop by id",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Workshops"
                ],
                "summary": "Get workshop by id",
                "parameters": [
                    {
                        "type": "string",
                        "description": "the id of the workshop",
                        "name": "workshopId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/rest.workshopByIdResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Update workshop",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Workshops"
                ],
                "summary": "Update workshop",
                "parameters": [
                    {
                        "type": "string",
                        "description": "the id of the workshop",
                        "name": "workshopId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Update workshop body",
                        "name": "updateWorkshopBody",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/rest.updateWorkshopValidated"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/httpResponse.IdResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Delete workshop (soft delete)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Workshops"
                ],
                "summary": "Delete workshop",
                "parameters": [
                    {
                        "type": "string",
                        "description": "the id of the workshop",
                        "name": "workshopId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Success Request",
                        "schema": {
                            "$ref": "#/definitions/httpResponse.IdResult"
                        }
                    },
                    "500": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/errorDomain.SmartError"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "domain.CreatedBy": {
            "type": "object",
            "required": [
                "id",
                "username"
            ],
            "properties": {
                "id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "username": {
                    "type": "string",
                    "example": "admin.smart"
                }
            }
        },
        "domain.Event": {
            "type": "object",
            "required": [
                "id",
                "name"
            ],
            "properties": {
                "code": {
                    "type": "string",
                    "example": "0001"
                },
                "id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "name": {
                    "type": "string",
                    "example": "EVENTO"
                }
            }
        },
        "domain.PaginationResults": {
            "type": "object",
            "required": [
                "current_page",
                "last_page",
                "size_page",
                "total"
            ],
            "properties": {
                "current_page": {
                    "type": "integer"
                },
                "from": {
                    "type": "integer"
                },
                "last_page": {
                    "type": "integer"
                },
                "size_page": {
                    "type": "integer"
                },
                "to": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "domain.SessionSums": {
            "type": "object",
            "properties": {
                "end_date": {
                    "description": "Description: the name of session",
                    "type": "string",
                    "example": "2023-11-10 08:10:00"
                },
                "id": {
                    "description": "Description: the id of session",
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "start_date": {
                    "description": "Description: the name of session",
                    "type": "string",
                    "example": "2023-11-10 08:10:00"
                },
                "total_payments": {
                    "description": "Description: the quantity of payments",
                    "type": "integer",
                    "example": 1
                },
                "total_presences": {
                    "description": "Description: the quantity of presences",
                    "type": "integer",
                    "example": 1
                },
                "total_registrations": {
                    "description": "Description: the quantity of registrations",
                    "type": "integer",
                    "example": 1
                }
            }
        },
        "domain.Workshop": {
            "type": "object",
            "required": [
                "created_by",
                "event",
                "id",
                "name",
                "workshop_type"
            ],
            "properties": {
                "capacity": {
                    "type": "integer",
                    "example": 2
                },
                "code": {
                    "type": "string",
                    "example": "0001"
                },
                "created_at": {
                    "type": "string",
                    "example": "2026-04-21 09:50:04"
                },
                "created_by": {
                    "$ref": "#/definitions/domain.CreatedBy"
                },
                "event": {
                    "$ref": "#/definitions/domain.Event"
                },
                "id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "name": {
                    "type": "string",
                    "example": "TALLERES - 1ER BLOQUE"
                },
                "shortname": {
                    "type": "string",
                    "example": "1ER BLOQUE"
                },
                "workshop_type": {
                    "$ref": "#/definitions/domain.WorkshopType"
                }
            }
        },
        "domain.WorkshopSums": {
            "type": "object",
            "properties": {
                "capacity": {
                    "description": "Description: the capacity of workshop",
                    "type": "integer",
                    "example": 1
                },
                "id": {
                    "description": "Description: the id of workshop",
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "name": {
                    "description": "Description: the name of workshop",
                    "type": "string",
                    "example": "Workshop 1"
                },
                "session_sums": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.SessionSums"
                    }
                },
                "total_payments": {
                    "description": "Description: the quantity of payments",
                    "type": "integer",
                    "example": 1
                },
                "total_presences": {
                    "description": "Description: the quantity of presences",
                    "type": "integer",
                    "example": 1
                },
                "total_registrations": {
                    "description": "Description: the quantity of registrations",
                    "type": "integer",
                    "example": 1
                }
            }
        },
        "domain.WorkshopType": {
            "type": "object",
            "required": [
                "code",
                "description",
                "id"
            ],
            "properties": {
                "code": {
                    "type": "string",
                    "example": "0001"
                },
                "description": {
                    "type": "string",
                    "example": "TIPO DE TALLER"
                },
                "id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                }
            }
        },
        "errorDomain.LayerErr": {
            "type": "string",
            "enum": [
                "domain",
                "infrastructure",
                "interface",
                "use_case"
            ],
            "x-enum-varnames": [
                "Domain",
                "Infra",
                "Interface",
                "UseCase"
            ]
        },
        "errorDomain.LevelErr": {
            "type": "string",
            "enum": [
                "info",
                "warning",
                "error",
                "fatal"
            ],
            "x-enum-varnames": [
                "LevelInfo",
                "LevelWarning",
                "LevelError",
                "LevelFatal"
            ]
        },
        "errorDomain.SmartError": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "error": {},
                "function": {
                    "type": "string"
                },
                "httpStatus": {
                    "type": "integer"
                },
                "layer": {
                    "$ref": "#/definitions/errorDomain.LayerErr"
                },
                "level": {
                    "$ref": "#/definitions/errorDomain.LevelErr"
                },
                "messages": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "raw": {
                    "type": "string"
                }
            }
        },
        "httpResponse.IdResult": {
            "type": "object",
            "required": [
                "data",
                "status"
            ],
            "properties": {
                "data": {
                    "type": "string",
                    "example": "201"
                },
                "status": {
                    "type": "integer"
                }
            }
        },
        "rest.createWorkshopValidated": {
            "type": "object",
            "required": [
                "event_id",
                "name",
                "type_id"
            ],
            "properties": {
                "capacity": {
                    "type": "integer",
                    "example": 1
                },
                "code": {
                    "type": "string",
                    "example": "0001"
                },
                "event_id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                },
                "name": {
                    "type": "string",
                    "example": "TALLERES - 1ER BLOQUE"
                },
                "shortname": {
                    "type": "string",
                    "example": "1ER BLOQUE"
                },
                "type_id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                }
            }
        },
        "rest.updateWorkshopValidated": {
            "type": "object",
            "required": [
                "name",
                "type_id"
            ],
            "properties": {
                "capacity": {
                    "type": "integer",
                    "example": 1
                },
                "code": {
                    "type": "string",
                    "example": "0001"
                },
                "name": {
                    "type": "string",
                    "example": "TALLERES - 1ER BLOQUE"
                },
                "shortname": {
                    "type": "string",
                    "example": "1ER BLOQUE"
                },
                "type_id": {
                    "type": "string",
                    "example": "739bbbc9-7e93-11ee-89fd-0242ac113422"
                }
            }
        },
        "rest.workshopByIdResult": {
            "type": "object",
            "required": [
                "status"
            ],
            "properties": {
                "data": {
                    "$ref": "#/definitions/domain.Workshop"
                },
                "status": {
                    "type": "integer"
                }
            }
        },
        "rest.workshopsResult": {
            "type": "object",
            "required": [
                "pagination",
                "status"
            ],
            "properties": {
                "data": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.Workshop"
                    }
                },
                "pagination": {
                    "$ref": "#/definitions/domain.PaginationResults"
                },
                "status": {
                    "type": "integer"
                }
            }
        },
        "rest.workshopsSummaryResult": {
            "type": "object",
            "required": [
                "data",
                "status"
            ],
            "properties": {
                "data": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.WorkshopSums"
                    }
                },
                "status": {
                    "type": "integer"
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

// SwaggerInfoworkshops holds exported Swagger Info so clients can modify it
var SwaggerInfoworkshops = &swag.Spec{
	Version:          "",
	Host:             "",
	BasePath:         "",
	Schemes:          []string{},
	Title:            "",
	Description:      "",
	InfoInstanceName: "workshops",
	SwaggerTemplate:  docTemplateworkshops,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfoworkshops.InstanceName(), SwaggerInfoworkshops)
}
