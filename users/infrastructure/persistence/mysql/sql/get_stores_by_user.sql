SELECT stores.id                  AS store_id,
       ...
       MAX(merchants.id)          AS merchant_id,
       MAX(merchants.name)        AS merchant_name,
       MAX(merchants.description) AS merchant_description
FROM (SELECT MAX(modules.id)          AS module_id,
    ...
    ((merchant_stores.policie_merchant_id IS NULL) OR
     (merchant_stores.policie_merchant_id = merchants.id)) AND merchants.deleted_at IS NULL
         LEFT JOIN core_stores AS stores ON
    ((merchant_stores.policie_store_id IS NULL AND merchants.id = stores.merchant_id) OR
     (merchant_stores.policie_store_id = stores.id)) AND stores.deleted_at IS NULL
WHERE stores.id IS NOT NULL
GROUP BY store_id, merchants.name, stores.name
ORDER BY  stores.name,merchants.name;
