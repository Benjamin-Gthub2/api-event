...
//go:embed sql/get_roles_by_user.sql
var QueryGetRolesByUser string

//go:embed sql/get_user_permissions_by_role.sql
var QueryGetUserPermissionsByRole string

//go:embed sql/get_global_roles_by_user.sql
var QueryGetGlobalRolesByUser string

//go:embed sql/get_password_user.sql
var QueryGetPasswordUser string

//go:embed sql/update_password_user.sql
var QueryUpdatePasswordUser string

//go:embed sql/update_user_theme.sql
var QueryUpdateUserTheme string

//go:embed sql/get_user_permissions_by_module.sql
var QueryGetUserPermissionsByModule string
...
func (r usersMySQLRepo) GetPasswordUser(
	...
	err = tx.QueryRowContext(
		ctx, QueryGetPasswordUser,
		userId,
	).Scan(&passwordTmp)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetPasswordUser").SetRaw(err)
	}
	password = &passwordTmp
	return password, nil
}

func (r usersMySQLRepo) UpdatePasswordUser(
	ctx context.Context,
	tx *sql.Tx,
	...
	_, err = tx.ExecContext(
		ctx,
		QueryUpdatePasswordUser,
		body.NewPassword,
		userId,
	)
	if err != nil {
		return r.err.Clone().SetFunction("UpdatePasswordUser").SetRaw(err)
	}
	return nil
}
...
func (r usersMySQLRepo) UpdatePasswordUserMain(
	ctx context.Context,
	body usersDomain.UpdatePasswordBody,
	userId string,
) (err error) {
	...
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return r.err.Clone().SetFunction("CreateUserMain").SetRaw(err)
	}
	tx, err = client.Begin()
	if err != nil {
		return err
	...
	currentPasswordOfDb, err := r.GetPasswordUser(ctx, tx, userId)
	if err != nil {
		return
	}

	if *currentPasswordOfDb == body.CurrentPassword {
		err = r.UpdatePasswordUser(ctx, tx, body, userId)
	} else {
		return usersDomain.ErrWrongPassword
	}
	if err = tx.Commit(); err != nil {
		return
	}
...
func (r usersMySQLRepo) GetRolesByUser(
	ctx context.Context,
	userId string,
) (
	merchants []usersDomain.MerchantRoleByUser,
	err error,
) {
	defer logErrorCoreDomain.PanicRecovery(&ctx, &err)
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetRolesByUser").SetRaw(err)
	}
	results, err := client.
		QueryContext(
			ctx,
	...
	rolesTmp := make([]MerchantRoleByUser, 0)
	err = carta.Map(results, &rolesTmp)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetRolesByUser").SetRaw(err)
	}
	automapper.Map(rolesTmp, &merchants)
	return merchants, nil
}

func (r usersMySQLRepo) GetUserPermissionsByRole(
	ctx context.Context,
	userId string,
	roleId string,
) (
	...
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserPermissionsByRole").SetRaw(err)
	}
	results, err := client.
		QueryContext(
			ctx,
			QueryGetUserPermissionsByRole,
			userId,
			roleId,
		)
	if err != nil {
	...
	permissionsTmp := make([]GetUserPermissionsByRole, 0)
	err = carta.Map(results, &permissionsTmp)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserPermissionsByRole").SetRaw(err)
	}
	automapper.Map(permissionsTmp, &permissions)
	return permissions, nil
}

func (r usersMySQLRepo) GetGlobalRolesByUser(
	ctx context.Context,
	userId string,
) (
	roles []usersDomain.RolesByUser,
	err error,
) {
	defer logErrorCoreDomain.PanicRecovery(&ctx, &err)
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetGlobalRolesByUser").SetRaw(err)
	}
	results, err := client.
		QueryContext(
			ctx,
			QueryGetGlobalRolesByUser,
			userId,
		)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetGlobalRolesByUser").SetRaw(err)
...
func (r usersMySQLRepo) UpdateUserTheme(
	ctx context.Context,
	...
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return r.err.Clone().SetFunction("UpdateUserTheme").SetRaw(err)
	}
	_, err = client.ExecContext(
		ctx,
		QueryUpdateUserTheme,
		body.Theme,
		userId,
	)
	if err != nil {
...
func (r usersMySQLRepo) GetUserPermissionsByModule(
	ctx context.Context,
	codeModule string,
	permissionId string,
) (
	userPermissions []usersDomain.UserPermissions,
	err error,
) {
	defer logErrorCoreDomain.PanicRecovery(&ctx, &err)
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserPermissionsByModule").SetRaw(err)
	}
	results, err := client.
		QueryContext(
			ctx,
			QueryGetUserPermissionsByModule,
			codeModule,
			permissionId)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserPermissionsByModule").SetRaw(err)
	...
	userPermissionsTmp := make([]UserPermissions, 0)
	err = carta.Map(results, &userPermissionsTmp)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserPermissionsByModule").SetRaw(err)
	}

	automapper.Map(userPermissionsTmp, &userPermissions)
	return userPermissions, nil
}

func (r usersMySQLRepo) GetUserByPersonId(
	ctx context.Context,
	personId string,
) (
	userByPersonId *usersDomain.User,
	err error,
) {
	defer logErrorCoreDomain.PanicRecovery(&ctx, &err)
	client, _, err := db.ClientDB(ctx)
	if err != nil {
		return nil, r.err.Clone().SetFunction("GetUserByPersonId").SetRaw(err)
	}
	results, err := client.
		QueryContext(
			ctx,
