...
func setDst(m *Mapper, dst reflect.Value, rsv *resolver) error {
	...
	for _, uid := range rsv.elementOrder {
		...
		//fmt.Printf("Tipo de dstIndirect: %s\n", dstIndirect.Type())
		//fmt.Printf("Valor de dstIndirect: %v\n", dstIndirect.Interface())
		//fmt.Printf("Tipo de elem.v: %s\n", elem.v.Type())
		//fmt.Printf("Valor de elem.v: %v\n", elem.v.Interface())

		if m.Crd == Collection {
			//if dstIndirect.Len() == dstIndirect.Cap() {
			//	newCapacity := dstIndirect.Cap() * 2
			//	newSlice := reflect.MakeSlice(dstIndirect.Type(), dstIndirect.Len(), newCapacity)
			//	reflect.Copy(newSlice, dstIndirect)
			//	dstIndirect.Set(newSlice)
			//}
